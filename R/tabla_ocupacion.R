#' Tabla de ocupación (CAT_OCU) por año y opcionalmente por entidad o municipio
#'
#' Devuelve una tabla con la distribución de homicidios según categoría ocupacional,
#' con opción de desagregación por año, entidad o municipio y por sexo.
#'
#' @param df Un data.frame que contenga las columnas `CAT_OCU`, `ANIO_OCUR`, `ANIO_REGIS`, `SEXO` y claves geográficas.
...
#'
#' @return Un data.frame con columnas para cada categoría ocupacional: `agricultor`, `artesanos`, `empleados`, `no_trabajaba`,
#' `comerciantes`, `operadores`, `directores`, `profesionales`, `no_especificado`, `no_aplica`, `seguridad`
#' @export
#' @importFrom dplyr filter group_by summarise n rename select across all_of
#' @importFrom tidyr pivot_wider
#' @importFrom magrittr %>%
tabla_ocupacion <- function(df, anio_tipo = NULL, desagregacion = NULL, geo_tipo = "ocurrencia", sexo = NULL, edad = NULL) {
  ...
  df %>%
    ...
    dplyr::select(dplyr::all_of(c(anio_col, geo_col)),
                  ...
                  operadores, directores, profesionales, no_especificado,
                  no_aplica, seguridad)
}
